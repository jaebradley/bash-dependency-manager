...
main() {
  ...
  local -r cache_dependenciers_path="${temp_directory}/cache"
  mkdir -p "${cache_dependenciers_path}" || fail "Unable to create directory at ${cache_dependenciers_path}"
  ...
  assert 255 "Installation script ${dependency_installation_script_path} is not a regular file" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
  ...
  assert 255 "Failed to read contents of target location file for dependency at ${dependency_directory_path}" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
  touch "${dependency_directory_path}/target" || fail "Unable to create target file"

  assert 255 "Failed to read contents of source location file for dependency at ${dependency_directory_path}" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
  touch "${dependency_directory_path}/source" || fail "Unable to create source file"

  assert 255 "Dependencies path at ${dependency_directory_path}/dependencies is not a regular file" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
  touch "${dependency_directory_path}/dependencies" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"

  assert 0 "" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
  assert 0 "foo has already been installed" install_dependency "${dependency_directory_path}" "${cache_dependenciers_path}"
}
